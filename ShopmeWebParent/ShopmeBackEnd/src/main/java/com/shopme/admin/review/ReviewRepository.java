...
public interface ReviewRepository extends SearchRepository<Review, Integer> {
    @Query("select r from Review r where r.headline like %?1% or r.comment like %?1% or "
            + "r.product.name like %?1% or concat(r.customer.firstName, ' ', r.customer.lastName) like %?1%")
